  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-800 uppercase italic tracking-tighter">Lançamentos de Vendas</h2>
          <p className="text-sm text-slate-500 font-bold uppercase tracking-widest text-[10px]">Registro e acompanhamento de serviços.</p>
        </div>
          ...
          <table className="w-full text-left min-w-[750px]">
            <thead className="bg-slate-50/50 border-b border-slate-100">
              <tr>
                ...
                <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center">Porte</th>
                <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center">Pagamento</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {billings.length === 0 ? (
                <tr><td colSpan={6} className="px-6 py-24 text-center text-slate-400 text-xs font-black uppercase italic opacity-50">Nenhum serviço registrado hoje.</td></tr>
              ) : (
                billings.map((b) => (
                  <tr key={b.id} className="group hover:bg-blue-50/40 transition-all">
                    <td className="px-6 py-5">
                      <div className="font-black text-slate-900 uppercase tracking-tight text-base leading-tight">{b.washType}</div>
                      <div className="flex items-center gap-1.5 mt-1.5">
                         <div className="bg-blue-50 px-2 py-0.5 rounded-md flex items-center gap-1">
                            <UserIcon size={10} className="text-blue-500" /> 
                            <span className="text-[9px] text-blue-600 font-black uppercase tracking-widest">{b.createdBy || 'Sistema'}</span>
                         </div>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <span className="inline-block px-3 py-1 rounded-xl text-[9px] font-black uppercase bg-slate-100 text-slate-600 border border-slate-200">{b.size}</span>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <span className="inline-block px-3 py-1 rounded-xl text-[9px] font-black uppercase bg-emerald-50 text-emerald-700 border border-emerald-100">{b.paymentMethod}</span>
                    </td>
                    <td className="px-6 py-5">
                       <div className="text-xs font-bold text-slate-600 flex items-center gap-1">
                          <Clock size={12} className="text-slate-400" /> {b.time}
                       </div>
                       <div className="text-[9px] font-bold text-slate-400 mt-0.5">{new Date(b.date).toLocaleDateString('pt-BR')}</div>
                    </td>
                    <td className="px-6 py-5 font-black text-emerald-700 italic text-lg">R$ {b.value.toFixed(2)}</td>
                    <td className="px-6 py-5 text-right">
                      <div className="flex justify-end gap-1">
                        <button onClick={() => handleEdit(b)} className="p-3 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors"><Plus size={20} className="rotate-45" /></button>
                        <button onClick={() => handleDelete(b.id)} className="p-3 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-colors"><Trash2 size={20} /></button>
                      </div>
                    </td>
                  </tr>
                ))
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl overflow-hidden animate-in slide-in-from-bottom duration-300">
             <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
               <h3 className="text-xl font-black text-slate-800 uppercase italic tracking-tighter">Lançar Novo Serviço</h3>
               <button onClick={() => setIsModalOpen(false)} className="p-2 bg-white shadow-sm rounded-full text-slate-400 hover:text-slate-600 transition-colors"><X size={20} /></button>
             </div>
             <form onSubmit={handleSubmit} className="p-8 space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                   <div className="sm:col-span-2">
                      <label className="text-[10px] font-black text-slate-700 uppercase mb-2 block tracking-widest ml-1">Descrição do Serviço</label>
                      <input 
                        required 
                        autoFocus
                   ...
                   <div>
                      <label className="text-[10px] font-black text-slate-700 uppercase mb-2 block tracking-widest ml-1">Valor do Serviço</label>
